    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidPage(
                box(width = 7,
                    ...
                    br(),
                    textInput("na.strings", "Missing values are written as: ", "NA"),
      ...
      tabItem(
        ...
        fluidPage(
          box(width=12,
              h2("Review the data"),
              div(style = 'overflow-x: scroll;overflow-y: scroll;max-height: 600px',
      ...
      tabItem(tabName = 'mvi',
        box(width=12,
            ...
            p("Imputation of missing values. Method: Impute median value."),
        ...
        fluidPage(
          box(width=12,
              ...
              p("Check if there are variables with too many categories and low 
                frequencies. Select the minimum frequency that you expect you variables to have.
                All variables with a lower frequency will be labeled with 'Other'"),
        ...
        fluidPage(
          box(width=10,
              p("Change categorical variables into numerical ones. Methods:
                Binary Encoding. Check the predictor variable to prevent its encoding."),
...
server <- function(input, output,session) {
  ...
  observeEvent(input$datafile,{
    data$orig_data <- read.csv(input$datafile$datapath,sep=input$sep,header=input$header,
  ...
  output$lvf_table <- renderTable({
    sd <- sapply(data$orig_data,function(x) if(is.numeric(x)) {sd(x,na.rm=T)}
                 else 1 )
  ...
  observeEvent(input$imputation, {
    dat <- data$orig_data %>%
      mutate_if(is.numeric, funs(ifelse(is.na(.), median(.,na.rm = T),.)))
